...
def maja(x,y,laius,kõrgus,pind,värv): 
    punktid=[(x,y-((2/3.0)*kõrgus)),(x,y),(x+laius,y),(x+laius,y-(2/3.0)*kõrgus),(x,y-((2/3.0)*kõrgus)),(x+laius/2.0,y-kõrgus),(x+laius,y-(2/3.0)*kõrgus)]
    suurus=random.randint(1,10) 
    pygame.draw.lines(pind,värv,False,punktid,suurus)

def uks(x,y,laius,kõrgus,pind,värv):
    punktid=[(x,y),(x,y-(1/2)*kõrgus),(x+(1/3)*laius,y-(1/2)*kõrgus),(x+(1/3)*laius,y)] 
    suurus=random.randint(1,10)
    pygame.draw.lines(pind,värv,False,punktid,suurus) 

def aken (x,y,laius,kõrgus,pind,värv): 
    punktid=[(x,y),(x,y-(1.3/2)*kõrgus),(x+(1/3)*laius,y-(1.3/2)*kõrgus),(x+(1/3)*laius,y)]
    suurus=random.randint(1,10)
    pygame.draw.lines(pind,värv,False,punktid,suurus) 
...
nuup=pygame.Rect(170,300,20,20)
